...
class CeeAgent {
    ...
    async processRequest(askedBy, question, chatHistory) {
        log.info(`[Cee] === Processing @Cee request ===`);
        log.info(`[Cee] Asked by: ${askedBy}`);
        log.info(`[Cee] Question: ${question}`);

        try {
            // Capture screenshot
            log.info('[Cee] Capturing screenshot...');
            const screenshot = await this._captureScreenshot();
            log.info('[Cee] Screenshot captured:', screenshot ? `${screenshot.length} chars base64` : 'FAILED');

            // Call appropriate API
            log.info('[Cee] Using provider:', this.provider);
            let response;
            ...
            log.info(`Cee responded: ${response}`);
            return response;

        } catch (err) {
            log.error('Cee request failed:', err.message);
            throw err;
        }
    }
    ...
    async _captureScreenshot() {
        ...
        if (this.mainWindow && !this.mainWindow.isDestroyed()) {
            try {
                ...
                if (screenshot) {
                    log.info('[Cee] Screenshot captured via renderer');
                    return screenshot;
                }
            } catch (err) {
                log.warn('[Cee] Renderer screenshot failed:', err.message);
            }
        }
        ...
        if (process.platform === 'linux') {
            try {
                ...
                log.warn('[Cee] Linux native screenshot failed:', err.message);
            }
        }

        // 3. Last resort: use desktopCapturer directly
        try {
            log.info('[Cee] Falling back to desktopCapturer...');
            ...
            if (sources.length === 0) {
                log.error('[Cee] No screen sources available');
                return null;
            }
            ...
            log.error('[Cee] desktopCapturer fallback failed:', err.message);
            return null;
        }
    }
        ...
        for (const tool of tools) {
            try {
                ...
                if (fs.existsSync(tmpFile)) {
                    ...
                    log.info(`[Cee] Screenshot captured using ${tool.cmd}`);
                    return base64;
                }
            } catch (err) {
                // Try next tool
    ...
    async _callOpenAI(question, screenshotBase64, chatHistory) {
        log.info('[Cee] Calling OpenAI API...');
        log.info('[Cee] API key prefix:', this.apiKey?.substring(0, 7) || 'NOT SET');
    ...
    async _callGemini(question, screenshotBase64, chatHistory) {
        log.info('[Cee] Calling Gemini API...');
